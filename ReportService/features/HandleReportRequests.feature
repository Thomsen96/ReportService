Feature: Handle report requests feature
  ...
  Scenario: Customer report invalid request is handled and an error message is returned
    ...
    #When the customer requests his report with session id "UniqueSessionId" without a payment in the log
    Then an error message "There are no logged payments for user: invalidCustomerId" is put on the messagequeue
